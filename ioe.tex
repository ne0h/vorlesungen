...
\begin{itemize}
	...
	\item \textbf{Klassisches Angreifermodell: "`Dolev-Yoa"'-Angreifer}
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textit{Objective Function}: Definiert die zu verwendenden Metriken und Constraints zur Berechnung des Node Rank (und damit welche Elternknoten gewählt werden sollen)
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item IPv6-Adressen
		\begin{itemize}
			\item Adressierung: Abbildung von \texttt{IPv6-Adressen} \(\rightarrow\) \texttt{IEEE 802.15.4-Adresse} (im Edge-Router). Alle Knoten eines LoWPANs haben einen gemeinsamen \texttt{IPv6-Prefix}
			\item Adresstypen
			\begin{itemize}
			...
			\end{itemize}
			\item Addressen bei Nutzung von \texttt{IEEE 802.15.4}: \texttt{MAC-Adresse} der Geräte (\texttt{IEEE-64-bit Adressen}) oder \texttt{16-bit} Kurzadressen, die der PAN-Koordinator verteilt (vgl. \texttt{802.15.4} im Non-Beacon-Modus). \texttt{IPv6-Adresse} setzt sich aus Prefix und Interface-Identifier zusammen
		\end{itemize}
		...
		\begin{itemize}
			\item Motivation: Verringerung des statischen Übertragungsaufwands durch Weglassen von redundanten oder weglassen unnötiger Informationen in höheren Schichten. Dabei kann der IPv6-Kopf oder der UDP-Kopf komprimiert werden
