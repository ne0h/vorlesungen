...
\begin{itemize}
	...
	\item Erwartungswert der Laufzeit: \(E\lbrack Laufzeit \rbrack: \sum_{ij} p_i \cdot M_{ij} \cdot q =: p^T \cdot M \cdot q\)
...
\paragraph{\(WERT(x)\)}
\begin{enumerate}
	\item Ermittle die Kindknoten \(x\) in stochastischer Reihenfolge
	\item Ermittle \(WERT(y)\)
	\item Falls \((w=1)\) gib \(0\) aus, ansonsten gib \((0 \downarrow WERT(z))\) aus
\end{enumerate}
