...
Konvexe Hüllen können als Schnitt dualer Halbräume bestimmt werden.
...
\[c := (\cdots,c_{-1}, c_0, c_1, \cdots) \in \mathbb{R}^{\mathbb{Z}}\]
zu unterteilen.

\subsubsection{Verdopplungsmatrix}
Jede Spalte wird jweils 2x auf die selbe Zeile abgebildet \(\rightarrow\) Verdoppelung.
\[d := D c :=
	\begin{bmatrix}
		\ddots 	& 		& 				& \\
				& 1 	&				& \\
...
Umformung mit Hilfe des Binomischen Lehrsatzes.\footnote{\url{http://de.wikipedia.org/wiki/Binomischer_Lehrsatz\#Binomischer_Lehrsatz_f.C3.BCr_nat.C3.BCrliche_Exponenten}}
...
\begin{itemize}
	\item \(\frac{1}{2}U_{n-1}\) ist das Differenzenschema
