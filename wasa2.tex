	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{enumerate}
				...
				\item Verschiedene Repräsentationen der Ressourcen, beispielsweise \texttt{HTML}, \texttt{JSON}, \texttt{XML}, etc; definierbar via \texttt{Content-Type} im \texttt{HTTP}-Header
				\item Navigation ausschließlich über \texttt{URLs}, die vom Server bereitgestellt werden (Hypermedia) \(\rightarrow\) lose Kopplung, da hartkodierte Pfade vermieden werden
			\end{enumerate}
		\end{itemize}
